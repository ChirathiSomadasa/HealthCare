...
router.get('/appointment/doctor', async (req, res) => {
    try {
        const doctorName = req.query.name; // Fetch doctor name from query parameters
        ...
        // Assuming the fees are stored in availableAppointments
        const fees = doctor.availableAppointments.length > 0 
            ? doctor.availableAppointments[0].fee // Taking the fee from the first appointment as an example
            : 'No appointments available';

        // Respond with doctor name and fees
        res.json({ doctorName: doctor.name, fees });
